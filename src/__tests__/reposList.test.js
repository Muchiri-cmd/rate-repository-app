...
//expect sth from first and second repo items
...
//       screen.debug()
      ...
      //test first repo Item
      const firstRepository = within(repositoryItems[0]);
      expect(firstRepository.getByText('jaredpalmer/formik')).toBeDefined();
      expect(firstRepository.getByText('Build forms in React, without the tears')).toBeDefined();
      expect(firstRepository.getByText('TypeScript')).toBeDefined();
      expect(firstRepository.getByText('1.6k')).toBeDefined()
      expect(firstRepository.getByText('21.9k')).toBeDefined();
      ...
      expect(secondRepository.getByText('Flexible promise-based React data loader')).toBeDefined()
      expect(secondRepository.getByText('JavaScript')).toBeDefined();
      expect(secondRepository.getByText('69')).toBeDefined();
      expect(secondRepository.getByText('1.8k')).toBeDefined();
